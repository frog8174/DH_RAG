...
class RecursiveTextSplitterLite:
    """
    輕量版 RecursiveCharacterTextSplitter：
    依照多層級分隔符（段落/換行/句號/逗號/空白），
    由粗到細遞迴切分，並加入實際 overlap。
    """
    ...
    def _split_recursively(self, text: str, seps: List[str]) -> List[str]:
        ...
        for p in parts:
            # 如果再加這一段會超過 chunk_size，就先把 current 收起來
            if current and len(current) + len(p) + len(sep) > self.chunk_size:
                if len(current) > self.chunk_size:
                    # 當前這塊自己也太長 -> 用下一層分隔符繼續遞迴切
    ...
    def split_text(self, text: str) -> List[str]:
        """執行分割並加入 overlap。"""
        base_chunks = self._split_recursively(text, self.separators)
